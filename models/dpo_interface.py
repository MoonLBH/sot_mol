...
from .dpo_diff import DPO_Lightning
from .interface import MolGen_Model


class MolGen_DPOModel(MolGen_Model):
    def __init__(
        self,
        atom_tokens,
        n_bond_types,
    ...
    def create_lightning_module(self, hparams=None, load_ckpt=None):
        ...
        if load_ckpt is not None:
            lightning_module = DPO_Lightning.load_from_checkpoint(
                load_ckpt,
                gen=self.network,
                vocab=self.vocab,
                map_location="cpu",
            ...
            lightning_module = DPO_Lightning(
                gen=self.network,
                vocab=self.vocab,
                **default_hparams,
            )
    ...
    def Train(
        ...
        checkpointing = ModelCheckpoint(
            ...
            save_top_k=3,
            every_n_epochs=1,
            monitor="train-dpo-reward-margin",
            mode="max",
            save_last=True,
        )
