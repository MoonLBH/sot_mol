...
from ..util.functional import zero_com,edges_from_nodes
...
def graph_distance_from_adj(adj_matrix, max_dist=6):
    """Compute shortest-path distances from an adjacency matrix.

    Args:
        adj_matrix (torch.Tensor): Adjacency, shape [batch_size, n_nodes, n_nodes], 1 for edge, 0 otherwise
        max_dist (int): Maximum distance to compute; unreachable pairs get max_dist + 1

    Returns:
        torch.Tensor: Graph distances, shape [batch_size, n_nodes, n_nodes], dtype long
    """

    if adj_matrix.dim() != 3:
        raise ValueError("adj_matrix must be a 3D tensor [batch, n_nodes, n_nodes]")

    batch_size, n_nodes, _ = adj_matrix.shape
    device = adj_matrix.device

    adj = adj_matrix.clone().bool()
    idx = torch.arange(n_nodes, device=device)
    adj[:, idx, idx] = False

    dist = torch.full((batch_size, n_nodes, n_nodes), max_dist + 1, device=device, dtype=torch.long)
    dist[:, idx, idx] = 0

    visited = torch.eye(n_nodes, device=device, dtype=torch.bool).unsqueeze(0)
    frontier = adj.clone()

    for k in range(1, max_dist + 1):
        newly_reached = frontier & (dist == (max_dist + 1))
        dist[newly_reached] = k

        visited = visited | frontier
        frontier = (frontier.float().bmm(adj.float()) > 0) & (~visited)

    return dist


def _make_time_gate(hidden_dim=16):
...
class EdgeMessages(torch.nn.Module):
    def __init__(self, d_model, d_message, d_out, n_coord_sets, d_ff=None, d_edge=None, eps=1e-6, graph_dist_max=6):
        super().__init__()
        ...
        self.d_edge = d_edge
        self.eps = eps
        self.graph_dist_max = graph_dist_max
        ...
        self.graph_proj = torch.nn.Linear(1, n_coord_sets, bias=False)
        self.t_gate = _make_time_gate()
    ...
    def forward(self, coords, node_feats, node_mask, edge_feats=None, flag_3Ds=None, graph_dists=None, t=None):
        ...
        Args:
            ...
            flag_3Ds (torch.Tensor): Flag for 3D usage, shape [batch_size, 1], 1 for 3D, 0 for 2D
            graph_dists (torch.Tensor): Graph distances, shape [batch_size, n_nodes, n_nodes]
            t (torch.Tensor): Diffusion time, shape [batch_size, 1] or [batch_size]
        ...
        # Apply flag_3Ds mask to coordinates
        if flag_3Ds is not None:
            flag_mask = flag_3Ds.repeat_interleave(self.n_coord_sets, dim=0).view(-1, 1, 1)
            coords = coords * flag_mask
            #print ('flag_3Ds in EdgeMessages:',flag_3Ds.shape,flag_mask.shape, coords.shape)
        ...
        if graph_dists is not None:
            graph_dists = graph_dists.unsqueeze(-1) if graph_dists.dim() == 3 else graph_dists
            graph_norm = (graph_dists.float() / float(self.graph_dist_max + 1)).clamp(0.0, 1.0)
            graph_feats = self.graph_proj(graph_norm)
            if t is not None:
                gate = self.t_gate(t.view(-1, 1)).view(-1, 1, 1, 1)
                coord_feats = (1.0 - gate) * coord_feats + gate * graph_feats
            else:
                coord_feats = coord_feats + graph_feats
...
class LengthsMLP(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_ff=None, graph_dist_max=6):
        super().__init__()

        d_ff = d_model * 4 if d_ff is None else d_ff

        self.graph_dist_max = graph_dist_max
        self.graph_node_proj = torch.nn.Linear(1, n_coord_sets, bias=False)
        self.t_gate = _make_time_gate()
    ...
    def forward(self, coord_sets, node_feats, node_mask=None, graph_dists=None, t=None):
        ...
        Args:
            coord_sets (torch.Tensor): Coordinate tensor, shape [batch_size, n_sets, n_nodes, 3]
            node_feats (torch.Tensor): Node feature tensor, shape [batch_size, n_nodes, d_model]
            node_mask (torch.Tensor): Node mask, shape [batch_size, n_sets, n_nodes] or [batch_size, n_nodes]
            graph_dists (torch.Tensor): Graph distances, shape [batch_size, n_nodes, n_nodes]
            t (torch.Tensor): Diffusion time, shape [batch_size, 1] or [batch_size]
        ...
        if graph_dists is not None and node_mask is not None:
            if node_mask.dim() == 3:
                node_mask = node_mask[:, 0, :]

            pair_mask = node_mask.unsqueeze(1) * node_mask.unsqueeze(2)
            graph_dists = graph_dists.float() * pair_mask
            denom = pair_mask.sum(dim=2, keepdim=True).clamp_min(1.0)
            node_graph = graph_dists.sum(dim=2, keepdim=True) / denom
            graph_norm = (node_graph / float(self.graph_dist_max + 1)).clamp(0.0, 1.0)
            graph_feats = self.graph_node_proj(graph_norm)
            if t is not None:
                gate = self.t_gate(t.view(-1, 1)).view(-1, 1, 1)
                lengths = (1.0 - gate) * lengths + gate * graph_feats
            else:
                lengths = lengths + graph_feats
...
class NodeFeedForward(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_ff=None, proj_sets=None, coord_norm="length", graph_dist_max=6):
        super().__init__()

        self.node_norm = torch.nn.LayerNorm(d_model)
        self.coord_norm = CoordNorm(n_coord_sets, norm=coord_norm)

        self.invariant_mlp = LengthsMLP(d_model, n_coord_sets, d_ff=d_ff, graph_dist_max=graph_dist_max)
        self.equivariant_mlp = EquivariantMLP(d_model, n_coord_sets, proj_sets=proj_sets)

    def forward(self, coord_sets, node_feats, node_mask, flag_3Ds=None, graph_dists=None, t=None):
        """Pass data through the layer
        ...
        # Apply flag_3Ds mask to coordinates
        if flag_3Ds is not None:
            coord_sets = coord_sets * flag_3Ds.view(-1,1,1,1)

        out_node_feats = self.invariant_mlp(
            coord_sets, node_feats, node_mask=node_mask, graph_dists=graph_dists, t=t
        )
...
class BondRefine(torch.nn.Module):
    def __init__(self, d_model, d_message, d_edge, d_ff=None, graph_dist_max=6):
        super().__init__()
        ...
        self.graph_dist_max = graph_dist_max
        self.graph_proj = torch.nn.Linear(1, 1, bias=False)
        self.t_gate = _make_time_gate()
    ...
    def forward(self, coords, node_feats, node_mask, edge_feats, flag_3Ds=None, graph_dists=None, t=None):
        ...
        if graph_dists is not None:
            ...
            if t is not None:
                gate = self.t_gate(t.view(-1, 1)).view(-1, 1, 1, 1)
                dists = (1.0 - gate) * dists + gate * graph_feats
            else:
                dists = dists + graph_feats
...
class EquiMessagePassingLayer(torch.nn.Module):
    ...
    def forward(
        ...
        # Apply flag_3Ds mask to coordinates at entry
        if flag_3Ds is not None:
            coords = coords * flag_3Ds.view(-1,1,1,1)


        coord_updates, node_updates = self.node_ff(
            coords, node_feats, node_mask, flag_3Ds=flag_3Ds, graph_dists=graph_dists, t=t
        )
        coords = coords + coord_updates
        node_feats = node_feats + node_updates

        # Apply flag_3Ds mask after coordinate update
        if flag_3Ds is not None:
            coords = coords * flag_3Ds.view(-1,1,1,1)
        ...
        # Apply flag_3Ds mask after attention update
        if flag_3Ds is not None:
            coords = coords * flag_3Ds.view(-1,1,1,1)
...
class EquiInvDynamics(torch.nn.Module):
    def __init__(
        ...
        self.final_ff_block = NodeFeedForward(
            d_model, n_coord_sets, coord_norm=coord_norm, graph_dist_max=graph_dist_max
        )
        self.coord_norm = CoordNorm(n_coord_sets, norm=coord_norm)
        self.feat_norm = torch.nn.LayerNorm(d_model)
        ...
        if self.bond_refine:
            self.refine_layer = BondRefine(d_model, d_message, d_edge, graph_dist_max=graph_dist_max)
    ...
    def forward(
        ...
        Args:
            ...
            flag_3Ds (torch.Tensor, Optional): Flag for 3D usage, shape [batch_size, 1], 1 for 3D, 0 for 2D
            graph_dists (torch.Tensor, Optional): Graph distances, shape [batch_size, n_nodes, n_nodes]
            t (torch.Tensor, Optional): Diffusion time, shape [batch_size, 1] or [batch_size]
        Returns:
        ...
        coords, inv_feats = self.final_ff_block(
            coords, inv_feats, atom_mask, flag_3Ds=flag_3Ds, graph_dists=graph_dists, t=t
        )
...
class DenoisingNet(torch.nn.Module):
    def __init__(
        ...
        in_feats = n_atom_feats + 8 +size_emb

        if self_cond:
            in_feats = in_feats + n_atom_feats 
        
        self.atom_emb =  torch.nn.Sequential(
            torch.nn.Linear(n_atom_feats, d_model), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_model, n_atom_feats)
        )

        self.t_emb = torch.nn.Sequential(
            torch.nn.Linear(1, d_model), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_model, 8)
        )
        ...
        self.atom_classifier_head = torch.nn.Sequential(
            torch.nn.Linear(d_model, d_model), torch.nn.SiLU(inplace=False), torch.nn.Linear(d_model, n_atom_feats)
        )
    ...
    def forward(
        ...
        bond_adj = (edge_feats.argmax(dim=-1) > 0).long()
        idx = torch.arange(bond_adj.size(1), device=bond_adj.device)
        bond_adj[:, idx, idx] = 0
        bond_adj = bond_adj * atom_mask.unsqueeze(1) * atom_mask.unsqueeze(2)
        graph_dists = graph_distance_from_adj(bond_adj, max_dist=self.graph_dist_max)
        ...
        size_emb = self.size_emb(n_atoms).expand(-1, atom_feats.size(1), -1)
        t_emb = self.t_emb(t).expand(-1, atom_feats.size(1), -1)
        atom_emb = self.atom_emb(atom_feats.float())

        inv_feats = torch.cat((atom_emb, t_emb, size_emb), dim=-1)
        if cond_atomics is not None:
            cond_atom_emb = self.atom_emb(cond_atomics.float())
            inv_feats = torch.cat((inv_feats, cond_atom_emb), dim=-1)
        ...
        # Default flag_3Ds to all ones (use 3D) if not provided
        batch_size = coords.size(0)
        flag_3Ds = torch.ones(batch_size, 1).to(coords.device) if flag_3Ds is None else flag_3Ds

        pred_coords, pred_feats, pred_edges = self.dynamics(
            coords,
            atom_feats,
            edge_feats,
            adj_matrix,
        ...
        # Apply flag_3Ds mask to final coordinates (output zeros for 2D samples)
        pred_coords = pred_coords * flag_3Ds.view(-1, 1, 1)
