...
from ..util.rdkit import _compute_single_mol_strain_energy
...
class RL_Lightning(SC_Lightning):
    ...
    def on_fit_start(self):
        ...
        if self.use_reference_anchor and self.ref_gen is None:
            ...
            self.prior_state = {name: param.detach().clone() for name, param in self.ref_gen.named_parameters()}
    ...
    def _compute_generation_quality_from_mols(self, mols, dtype, device):
        ...
        for mol in valid_mols:
            ...
            # ---- strain energy ----
            strain_res = _compute_single_mol_strain_energy(mol, use_heavy_atom_norm=True)
            if strain_res["strain_energy"] is not None:
                strain_energies.append(strain_res["strain_energy"])
                strain_energies_per_atom.append(strain_res["strain_energy_per_atom"])
                n_strain_success += 1
        ...
        mean_strain = float(np.mean(strain_energies)) if len(strain_energies) > 0 else float("nan")
        median_strain = float(np.median(strain_energies)) if len(strain_energies) > 0 else float("nan")

        mean_strain_per_atom = (
            float(np.mean(strain_energies_per_atom))
            if len(strain_energies_per_atom) > 0
            else float("nan")
        )
        median_strain_per_atom = (
            float(np.median(strain_energies_per_atom))
            if len(strain_energies_per_atom) > 0
            else float("nan")
        )
        ...
        return {
            ...
            # strain metrics
            "strain-energy-mean": torch.tensor(mean_strain, dtype=dtype, device=device),
            "strain-energy-median": torch.tensor(median_strain, dtype=dtype, device=device),
            "strain-energy-per-atom-mean": torch.tensor(mean_strain_per_atom, dtype=dtype, device=device),
            "strain-energy-per-atom-median": torch.tensor(median_strain_per_atom, dtype=dtype, device=device),
            "n-strain-success": torch.tensor(float(n_strain_success), dtype=dtype, device=device),
        }
    ...
    def compute_l2_regularization(self):
        ...
        for name, param in self.gen.named_parameters():
            total += (param - self.prior_state[name]).pow(2).sum()
    ...
    def FM_training_step(self, batch):
        ...
        if self.regularization_type == "Parametric_L2":
            anchor_loss = self.compute_l2_regularization()
        ...
        self.log("train-gen-strain-energy-mean", quality_metrics["strain-energy-mean"], on_step=True, on_epoch=True, logger=True, sync_dist=True)
        self.log("train-gen-strain-energy-median", quality_metrics["strain-energy-median"], on_step=True, on_epoch=True, logger=True, sync_dist=True)
        self.log("train-gen-strain-energy-per-atom-mean", quality_metrics["strain-energy-per-atom-mean"], on_step=True, on_epoch=True, logger=True, sync_dist=True)
        self.log("train-gen-strain-energy-per-atom-median", quality_metrics["strain-energy-per-atom-median"], on_step=True, on_epoch=True, logger=True, sync_dist=True)
        self.log("train-gen-n-strain-success", quality_metrics["n-strain-success"], on_step=True, logger=True, sync_dist=True)
