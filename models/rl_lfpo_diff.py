...
class LFPOF_Lightning(RL_Lightning):
    ...
    def FM_training_step(self, batch):
        ...
        flag_3Ds = expanded_batch["flag_3Ds"]

        interp_data = self.interpolate(expanded_batch, t_flat, flag_3Ds=flag_3Ds)
        ...
        ref_predicted = self._compute_reference_predictions(interp_data, t_flat, cond_batch, flag_3Ds)
        ...
        type_loss_ps = pull_w_flat * type_plus + (1.0 - pull_w_flat) * type_minus
        type_rect_loss = type_loss_ps.mean()
        ...
        bond_loss_ps = pull_w_flat * bond_plus + (1.0 - pull_w_flat) * bond_minus
        bond_rect_loss = bond_loss_ps.mean()

        charge_rect_loss = torch.zeros_like(type_rect_loss)
        delta_charge_abs = torch.zeros_like(type_rect_loss)
        use_charge = (
        ...
        if use_charge:
            ...
            charge_loss_ps = pull_w_flat * charge_plus + (1.0 - pull_w_flat) * charge_minus
            charge_rect_loss = charge_loss_ps.mean()
            delta_charge_abs = delta_charge_ps.mean()
        ...
        coord_loss_ps = pull_w_flat * coord_plus + (1.0 - pull_w_flat) * coord_minus
        coord_rect_loss = coord_loss_ps.mean()
        ...
        lfpof_main_loss = (
            ...
            + (self.lfpo_lambda_charges_rect * charge_rect_loss if use_charge else 0.0)
        )

        if self.formulation == "endpoint":
            coords_target = expanded_batch["real_coords"]
        else:
            coords_target = expanded_batch["real_coords"] - expanded_batch["noise_coords"]

        aux_target = {
            "coords": coords_target,
            "atomics": expanded_batch["real_atomics"],
            "bonds": expanded_batch["real_bonds"],
            "charges": expanded_batch["real_charges"],
            "masks": expanded_batch["masks"],
        }
        ...
        self.log("train-lfpof-delta-type-abs-mean", delta_type_abs.mean(), on_step=True, logger=True, sync_dist=True)
        self.log("train-lfpof-delta-bond-abs-mean", delta_bond_abs.mean(), on_step=True, logger=True, sync_dist=True)
        self.log("train-lfpof-delta-coord-abs-mean", delta_coord_abs.mean(), on_step=True, logger=True, sync_dist=True)
        if use_charge:
            self.log("train-lfpof-delta-charge-abs-mean", delta_charge_abs, on_step=True, logger=True, sync_dist=True)
