...
def compute_type_stats(mgs, natom_types, nbond_types, eps=1e-6):
    atom_counts = torch.zeros(natom_types)
    bond_counts = torch.zeros(nbond_types)

    for mg in mgs:
        atom_counts += mg.atomics.float().sum(0)

        if hasattr(mg, "bond_types") and mg.bond_types is not None:
            bond_counts += mg.bond_types.float().sum(0)

    atom_probs = atom_counts + eps
    atom_probs = atom_probs / atom_probs.sum()

    bond_probs = bond_counts + eps
    bond_probs = bond_probs / bond_probs.sum()

    return atom_probs, bond_probs