...
def mol_from_atoms(
    coords: ArrT, tokens: list[str], bonds: Optional[ArrT] = None, charges: Optional[ArrT] = None, pharmcophores: Optional[ArrT] =None, sanitise=True
):
    ...
    for bond in bonds.astype(np.int32).tolist():
        ...
        if b_type not in GP.IDX_BOND_MAP:
            return None

        # Don't add self connections
        if start != end:
            b_type = GP.IDX_BOND_MAP[b_type]
    ...
    try:
        ...
        for aid,atom in enumerate(mol.GetAtoms()):
            ...
            atom.SetProp("PHARM", pharmcophores[aid] if pharmcophores is not None else "BASIC")
    except Exception:
        return None
...
def get_pharmcore_informations(mol: Chem.rdchem.Mol):
    """
    Get pharmacophore information from an RDKit molecule.
    Returns a dictionary with keys as pharmacophore families and values as lists of atom indices.
    """
    fdefName = os.path.join(RDConfig.RDDataDir,'BaseFeatures.fdef')
    factory = ChemicalFeatures.BuildFeatureFactory(fdefName) 
    feats = factory.GetFeaturesForMol(mol)
    
    pharmcophore_infos = {'Donor':[],
                           'Acceptor':[],
                           'NegIonizable':[],
                           'PosIonizable':[],
                           'Aromatic':[],
    ...
    for feat in feats:
        ...
        if family in pharmcophore_infos.keys():
            pharmcophore_infos[family].append(atomids)
    
    ringinfo=mol.GetRingInfo()
    pharmcophore_infos['Ring']=[list(x) for x in ringinfo.AtomRings()]
    
    for aid,atomic_num in enumerate(mol.GetAtoms()):
        if atomic_num.GetAtomicNum() in [9, 17,35,53]:
            pharmcophore_infos['Halogen'].append([aid])
    
    return pharmcophore_infos
